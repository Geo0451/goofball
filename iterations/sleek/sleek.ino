...
void checkTouchLogic() {
  ...
  // Short Release (Switch Personality)
    ...
    if (millis() - touchStartTime < 500 && !touchHandled) {
      ...
      // If tapped during startup, just dismiss the startup screen instead of sliding
...
void drawStartupAnimation() {
  // Auto-transition to IDLE after 3 seconds so the first personality actually rests on screen
